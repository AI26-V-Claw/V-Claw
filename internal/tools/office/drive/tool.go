...
var RegistryEntries = []ToolRegistryEntry{
	...
	{Name: ToolNameExportFile, Owner: "integration", Description: "Export a Google Workspace native file (Docs, Sheets, Slides, etc.) into the tool response as readable text or another export MIME type. Use this when the goal is to read or summarize content from a Google-native file and no local path is needed. Read-only; does not write local files.", DefaultRiskLevel: "safe_read", RequiresApproval: false},
	{Name: ToolNameDownloadFile, Owner: "integration", Description: "Download a Drive file's content into the tool response with a size cap. Use this when you need inline content from a binary or non-Google-native Drive file. Google Docs Editors files are auto-exported to a readable format when needed, but for a deliberate 'export this Doc/Sheet/Slide to text' request prefer drive.exportFile. Read-only; does not write local files.", DefaultRiskLevel: "safe_read", RequiresApproval: false},
	{Name: ToolNameSaveFile, Owner: "integration", Description: "Save a Drive file to the local sandbox workspace and return the host Path. Use this before sandbox.extractPDF, sandbox.runPython, sandbox.runShell, or any tool that needs localPath for a Drive file. Works for any file type — Google Docs Editors files are auto-exported. Omit outputDir to save into the workspace root. Writes a local file and requires approval.", DefaultRiskLevel: "local_write", RequiresApproval: true},
...
func (t DriveTool) Description() string {
	...
	case ToolNameExportFile:
		return "Export a Google Workspace native file, such as a Doc, Sheet, or Slide, into the tool response with a size cap. Use this when you want readable exported content from a Google-native file and do not need a local path. This is read-only and does not write local files."
	case ToolNameDownloadFile:
		return "Download a Drive file's content into the tool response with a size cap. Use this for binary or non-Google-native files when you want the content inline in the tool response. Google Docs Editors files (Docs, Sheets, Slides) are automatically exported to a text-friendly format when needed, but if the user explicitly wants exported text from a Google-native file, prefer drive.exportFile. This is read-only and does not write local files. Do not use this before sandbox.extractPDF or any tool needing localPath; use drive.saveFile instead."
	case ToolNameSaveFile:
		return "Save a Drive file onto the local disk in the sandbox workspace and return the host Path. Use this before sandbox.extractPDF, sandbox.runPython, sandbox.runShell, or any tool that needs localPath for a Drive file; do not invent workspace paths from Drive filenames. Works for any file type — Google Docs Editors files are auto-exported. outputDir is optional and defaults to the workspace root; the path is confined to the workspace. Writes a local file and requires human approval."
