...
func (RunPythonTool) Description() string {
	return "Run Python code or a workspace-relative Python script inside the configured sandbox. Prefer sandbox.extractPDF when PDF structure must be preserved for Google Docs; use runPython for custom PDF analysis, Excel/CSV processing with pandas, or Word documents with python-docx. For PDF or long-document summarization, do not print the full extracted text; keep stdout bounded, preferably under 4000 characters, by printing page counts, total extracted length, and short page/section snippets or chunks. If full extraction is needed, write it to a workspace file and print only the file path plus a short preview. Available libraries: fitz/PyMuPDF, pdfplumber, pandas, numpy, openpyxl, xlrd, python-docx, chardet, PyYAML. The result includes workspace_dir — the absolute host path where files persist between runs. To reference a file created or found in the workspace (e.g. for chat.sendMessage attachments), use workspace_dir + filename. This code execution action requires approval."
}
